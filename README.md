# 🚀个性化简历生成 Resume Skill

`lobster-resume` 是一个用于“保存候选人基础履历 + 根据 JD 头脑风暴 + 默认生成可编辑 Word 简历”的 Skill。

它支持多套 Word 简历模板和预览图，覆盖金融、投行、财务会计、软件、咨询、运营、销售、法律、设计、行政、人力资源、外贸、通信电子、通用简洁版等方向。为了让用户安装更快，主仓库不再默认打包全部模板；生成简历前，skill 会先根据 JD 的公司类型、岗位方向和候选人背景判断模板方向，需要真实模板时再按类别下载。
...
- 根据公司/岗位风格选择模板方向、简历表达方式和排版风格；模板按需下载，不强制用户一次性下载全部模板。
...
5. **多模板选择**
   ...
   模板库包括 28 个类别、100+ Word 模板以及对应预览图。轻量安装默认不下载模板实体，生成前会先选择适合岗位的模板方向，需要时再下载对应类别，例如：
...
```bash
git clone --depth 1 https://github.com/chw7crhd13-bit/resume_skill.git
cd resume_skill
```

推荐使用 `--depth 1`，这样不会下载早期提交里的完整模板历史。
...
模板按需下载需要能访问 GitHub raw/API 地址。如果网络不可用，skill 仍会使用本地 Word 渲染器生成 `.docx`。
